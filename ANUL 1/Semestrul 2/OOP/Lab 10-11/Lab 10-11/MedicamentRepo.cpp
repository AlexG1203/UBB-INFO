...
void MedicamentRepo::modifyMedicamentRepoDen(const Medicament& med_de_modificat) {
	bool isGasit = false;
	int poz = -1;
	string denumire = med_de_modificat.getDenumire();
	const auto it = std::find_if(all.begin(), all.end(), [&](const Medicament& med) {
		poz++;
	...
	if (isGasit) {
		all[poz] = med_de_modificat;
	}
	if (!isGasit) {
		throw MedicamentRepoException("Medicament inexistent!\n");
	}
}
...
const Medicament& MedicamentRepo::srcMedicamentRepo(int id) {
	...
	throw MedicamentRepoException("ID invalida!");
}

const Medicament& MedicamentRepo::srcMedicamentRepoDenumire(const std::string& denumire) {
	/*if (id >= 0) {
		for (int i = 0; i < (int)all.size(); i++) {
			if (all[i].getID() == id)
				return all[i];
