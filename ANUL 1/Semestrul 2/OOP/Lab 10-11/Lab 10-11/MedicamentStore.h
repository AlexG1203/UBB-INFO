...
class MedicamentStore {
	...
	//Undo
	std::vector<unique_ptr<ActiuneUndo>> undoAction;
...
public:
	...
	*/
	...
	void modifyMedicamentDen(int id, const string& denumire_noua, int pret_nou, const string& producator_nou, const string& substanta_noua);
	
	//Cauta medicament dupa id
	const Medicament& srcMedicament(int id);

	//Cauta medicament dupa denumire
	const Medicament& srcMedicamentDenumire(string denumire);
